...
const CampaignUpdate = () => {
  ...
  const camp_id = localStorage.getItem("campaign_id")
  ...
  useEffect(async () => {
    ...
    try {
      ...
      if (data && statusCode == 200) {
        ...
        setCampaignObjective(data.campaign_objective);
        setCampaignDescription(data.campaign_description);
        setCampaignIndustry(data.campaign_industry);
        setLocation(data.location);
        setCampaignStartDate(data.campaign_start_date);
        setCampaignEndtDate(data.campaign_end_date);
        setTotalBudget(data.total_budget);
        setPaymentStructure(data.payment_structure);
        setPreferdInfluencerType(data.prefered_influencer_type);
        setPreferedPlatform(data.prefered_platform);
        setFollowerCount(data.followers_count);
        setEngagementRate(data.engagement_rate);
        setInfluencerNiche(data.influencer_niche);
        setNumberOfInfluencers(data.no_of_influencers);
        setExpectedRevenue(data.expected_revenue);
        setContentType(data.content_type);
        setNumberOfPosts(data.number_of_posts);
        setHashTagUsed(data.hash_tag_used);
        setMentionedTags(data.mentioned_tags);
        setApprovalRequired(data.approval_required);
      }
    } catch (error) {
      console.log("error in get campaign details api call", error);
    }
  }, []);
  ...
  const updateCampaign = () => {
    ...
    console.log("campagin name", campaign_name);
    let CampaignDetails = {
    ...
    dispatch(updateCampaignAction(CampaignDetails, token));
  };
